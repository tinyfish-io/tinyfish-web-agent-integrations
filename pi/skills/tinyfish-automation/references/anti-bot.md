...
If you can't watch the run, step screenshots and HTML snapshots tell you the same story after the
fact — enable `capture_config.screenshots` and `capture_config.snapshots` **on the next run you
were going to make anyway**.

Do not re-run purely to collect them. A run that errored or timed out may still be executing, and a
